...
export class GameRoom extends Room<GameRoomState> {
    ...
    handleSubmitHand(client: Client, message: { slots: string[] }) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (cId && handMap.has(cId)) {
                ...
                player.slots.push(this.createEmptyCard()); // Push explicit empty card object
            }
        }
    ...
    // Helper for empty card
    createEmptyCard(): Card {
        const c = new Card();
        c.id = "empty";
        c.type = "support"; // Dummy type to avoid errors
        return c;
    }
    ...
    calculateOutcome() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (!c1 && !c2) {
                ...
                // Direct Hit P2 -> P1
                if (c2!.type === 'offense') p1DmgTaken += c2!.value; // c2 is definitely defined here due to check above
                if (c2!.type === 'defense') p2Shield += c2!.value;
                if (c2!.type === 'support' && c2!.templateId === 'heal') p2Shield += c2!.value;
            } else if (!c2) {
                // Direct Hit P1 -> P2
                ...
                if (c1.type === 'offense' && c2.type === 'offense') {
                    ...
                    p2DmgTaken += c1.value; // Attack hits the barrier (damage to shield)
                    p2Shield += c2.value;
                    res.interaction = 'counter';
                } else if (c1.type === 'defense' && c2.type === 'offense') {
                    p2DmgTaken += Math.round(c2.value * 0.5); // Reflect
                    p1DmgTaken += c2.value; // Attack hits the barrier (damage to shield)
                    p1Shield += c1.value;
                    res.interaction = 'counter';
                } else {
